...
@"extern void asmGetLowestF(int *iteration, float *NodeArrayF);"

.THUMB_FUNC		@this function gets the lowest f from a list of arrays
asmGetLowestF:			
...
@"extern void addSprites(int* start, int* Finish, int* characterAddress, int *spriteData);"
	...
	lowestFFor: 		@this is where it jumps back to if the numbers are not equal
	...
	add r4, #1		@add 1 to r4 indicating it has moved another step in the loop
	cmp r4, r5 		@compares the current loop with the end of the loop
	bne lowestFFor	@jumps back to top of for loop
